...
} from "../../../utils/apiHandler/playListHandler";
...
export const Modal = ({ id, setModal, video, from, playListid }) => {
  ...
  const isWatchLater = watchLaterState.watchLaterVideos.some(
    (item) => item?.video?._id === id
  );
  const isVideoLiked = state.likedVideos.some(
    (item) => item?.video?._id === id
  );
      ...
      <div className="modal-container">
        {from === "playlistcard" && (
          <div
            ...
            onClick={() => deletePlaylist(playListid, playlistDispatch)}
          >
        ...
        {from === "playlistfolder" && (
          <div
            className="modal-unit flex align-center bold-highlight-color"
            onClick={() =>
              deleteVideoFromPLaylist(playListid, video._id, playlistDispatch)
            }
          >
            <span className="material-icons">delete</span>
          ...
          <>
            ...
            {isWatchLater ? (
              <div
                className="modal-unit flex align-center bold-highlight-color"
                onClick={() =>
                  deleteWatchLaterVideo(video._id, watchLaterDispatch)
                }
              >
